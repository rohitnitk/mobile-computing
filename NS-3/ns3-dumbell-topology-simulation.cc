...
Senders	|	H2------R1------R2-----H5	 |	Receivers
	...
	Links:
	H1R1/H2R1/H3R1/H4R2/H5R2/H6R2: P2P with 100Mbps and 5ms.
	R1R2: (dumbbell bridge) P2P with 20Mbps and 50ms.
	packet size: 1.2KB.
	Number of packets decided by Bandwidth delay product:
	i.e. #packets = Bandwidth*Delay(in bits)
	Therefore, max #packets (HiRj) = 100Mbps*5ms = 500000 //  NOT USED
	and max #packets (R1R2) = 20Mbps*50ms =1000000   // NOT USED
*/
#include <string>
#include <fstream>
#include <cstdlib>
...
void partAC() {
	...
	std::string latencyHR = "5ms";
	std::string rateRR = "20Mbps";
...
//	p2pRR.SetQueue("ns3::DropTailQueue", "MaxPackets", UintegerValue(queueSizeRR));
	...
	*/
	...
	/*
		...
		by PointToPointHelper::SetChannelAttribute, then, forLT: this type stands for "limited transfer" and allows to specify traffic which is sent as fast as possible. Only TCP traffic is allowed. The amount of data to tran each node in the input container,
	...
	********************************************************************/
	/********************************************************************
		One flow for each connection and measure
	...
	********************************************************************/
	...
	Ptr<Socket> ns3TcpSocket1 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(0), port), port, "TcpVegas", senders.Get(0), receivers.Get(0), netDuration, netDuration+durationGap, packetSize, numPackets, transferSpeed, netDuration, netDuration+durationGap);
	...
	Ptr<Socket> ns3TcpSocket2 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(1), port), port, "TcpVegas", senders.Get(1), receivers.Get(1), netDuration, netDuration+durationGap, packetSize, numPackets, transferSpeed, netDuration, netDuration+durationGap);
	...
	Ptr<Socket> ns3TcpSocket3 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(2), port), port, "TcpVegas", senders.Get(2), receivers.Get(2), netDuration, netDuration+durationGap, packetSize, numPackets, transferSpeed, netDuration, netDuration+durationGap);
		...
		*/
		if(t.sourceAddress == "10.1.0.1") {
			if(mapDrop.find(1)==mapDrop.end())
				mapDrop[1] = 0;
			*stream1PD->GetStream() << "TcpVegas 1st Flow " << i->first  << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
			...
			*stream1PD->GetStream() << "Max throughput: " << mapMaxThroughput["/NodeList/5/$ns3::Ipv4L3Protocol/Rx"] << std::endl;
			...
			if(mapDrop.find(2)==mapDrop.end())
				mapDrop[2] = 0;
			*stream2PD->GetStream() << "TcpVegas 2nd Flow " << i->first  << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
			...
			*stream2PD->GetStream() << "Max throughput: " << mapMaxThroughput["/NodeList/6/$ns3::Ipv4L3Protocol/Rx"] << std::endl;
			...
			if(mapDrop.find(3)==mapDrop.end())
				mapDrop[3] = 0;
			*stream3PD->GetStream() << "TcpVegas 3rd Flow " << i->first  << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
